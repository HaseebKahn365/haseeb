...
class ChatNotifier extends StateNotifier<ChatState> {
  ...
  Future<dynamic> _executeFunction(FunctionCall call) async {
    ...
    switch (call.name) {
      ...
      case 'listWishlistItems':
        try {
          final args = call.args as Map<String, dynamic>;
          final includeCompleted = args['includeCompleted'] as bool? ?? true;
          final type = (args['type'] as String?)?.toLowerCase();
          final dueBeforeStr = args['dueBeforeStr'] as String?;

          DateTime? dueBefore;
          if (dueBeforeStr != null && dueBeforeStr.trim().isNotEmpty) {
            try {
              dueBefore = DateFormat('yyyy-MM-dd').parseStrict(dueBeforeStr);
            } catch (e) {
              return jsonEncode({
                'success': false,
                'error': 'Invalid dueBeforeStr format, expected yyyy-MM-dd',
                'details': e.toString(),
              });
            }
          }
          ...
          final result = items
              .map(
                (w) => {
                  ...
                  'dueDate': DateFormat('yyyy-MM-dd').format(w.dueDate),
                  'type': w.type,
                  'count': w.count,
                  'duration': w.duration,
                },
          ...
          dev.log('listWishlistItems: error -> $e');
          return jsonEncode({
            'success': false,
            'error': e.toString(),
            'args': call.args,
      ...
      case 'updateWishlistItem':
        try {
          ...
          // If id is not provided, return minimal list for selection
          if (id == null || id.isEmpty) {
            final repo = await WishlistRepository.init();
